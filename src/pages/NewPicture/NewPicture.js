...
class NewPicture extends React.Component {
  ...
  componentDidMount() {
    this.$f7router.on('routeChange', newRoute => {
      const isCurrentPage = newRoute.name === 'NewPicture'
      if (isCurrentPage) {
  ...
  startLivePreview() {
    ...
    videoRef.addEventListener('play', e => {
      ...
      placeholderRef.classList.remove('visible')
    })
    this.props
  ...
  takePicture() {
    const videoRef = document.querySelector('#cnh-picture-live-preview')
    const placeholderRef = document.querySelector('#cnh-picture-placeholder')
    videoRef.classList.remove('visible')
    placeholderRef.classList.add('visible')
    this.props
  ...
  render() {
    ...
    return (
      <Page className="new-picture-page">
        ...
        <div className="new-picture-page__content">
          <div
            id="cnh-picture-placeholder"
            className="new-picture-page__placeholder visible">
            <Preloader
              size={32}
              className="new-picture-page__placeholder__loader"
            />
          </div>
          {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
          <video
            id="cnh-picture-live-preview"
            className="new-picture-page__video-preview"
          />
        </div>
        <div className="new-picture-page__footer">
          <PrimaryButton
            className="recording"
            onClick={() => {
              this.takePicture()
